...
class OrderManager:
    ...
    def create_order(self, symbol: str, side: str, order_type: str,
        ...
        try:
            ...
            self.logger.info(f"创建订单: {order_id}, {symbol} {side} {size} @ {price or '市价'}")
            ...
            if result:
                ...
                if okx_order_data:
                    ...
                    if s_code == '0' and okx_ord_id:
                        ...
                        self.logger.info(f"订单已提交成功: OKX订单ID={okx_ord_id} (原ID={old_order_id})")
                    else:
                        # 订单提交失败
    ...
    def update_order_status(self, order: Order) -> Order:
        ...
        try:
            ...
            if order.order_id.startswith(order.symbol + '_') or not order.order_id.isdigit():
                self.logger.debug(f"订单ID是内部生成的，尚未提交到交易所: {order.order_id}")
                # 如果订单状态是SUBMITTED但没有真实订单ID，可能是提交失败
                if order.status == OrderStatus.SUBMITTED:
                    self.logger.warning(f"订单状态为SUBMITTED但订单ID是内部ID，可能提交失败或未更新")
                    # 尝试重新提交或标记为失败
                    order.status = OrderStatus.REJECTED
                return order
            ...
            if result:
                ...
                self.logger.debug(f"订单状态已更新: {order.order_id} = {order.status.value}")
