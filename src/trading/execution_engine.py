...
class ExecutionEngine:
    ...
    async def execute_decision(self, decision: TradingDecision) -> Optional[Order]:
        ...
        try:
            ...
            self.logger.info(
                f"开始执行决策: {decision.symbol} {decision.action} {decision.position_size:.2%}"
            )
            ...
            if decision.action in ["long", "short"]:
                # 检查持仓和委托
                check_result = await self._check_before_open_position(
                    symbol, decision.action, position_side
                )
            ...
            if price and price > 0:
                ...
                self.logger.info(
                    ...
                    f"差价={(price - current_market_price) / current_market_price * 100:.2f}%"
                ...
                price = None  # 市价单不需要价格
                self.logger.info(
                    f"[订单类型] {symbol}: {action_desc} | 使用市价单（未计算最佳价格）"
                )
            ...
            try:
                    ...
                    if balance_data:
                        ...
                        if isinstance(balance_data, dict):
                                    ...
                                    for item in data:
                                        ...
                                        if details:
                                            for detail in details:
                                                if detail.get(
                                                    "ccy"
                                                ) == "USDT" and detail.get("availBal"):
                                                    available_balance = float(
                                                        detail.get("availBal", 0)
                                                    )
                                                    break
                            ...
                            for item in balance_data:
                                ...
                                if details:
                                    for detail in details:
                                        if detail.get("ccy") == "USDT" and detail.get(
                                            "availBal"
                                        ):
                                            available_balance = float(
                                                detail.get("availBal", 0)
                ...
                if available_balance == 0:
                    ...
                    default_balance = self.config_mgr.get_config(
                        "trading", "auto_trading", {}
                    ).get("default_balance", 10000.0)
                ...
                if price and price > 0:
                    ...
                    try:
                        ...
                        if (
                            ...
                            and len(instruments) > 0
                        ):
                            instrument_info = instruments[0]
                            lot_size = float(
                                instrument_info.get("lotSz", 1.0)
                            )  # 最小下单单位
                            min_size = float(
                                instrument_info.get("minSz", 0.001)
                            )  # 最小订单数量
                            ct_val = float(
                                instrument_info.get("ctVal", 1.0)
                            )  # 合约面值
                    ...
                    if size < min_size:
                        self.logger.warning(
                            f"计算出的数量({size})小于最小订单数量({min_size})，调整到最小值"
                        )
                        size = min_size
                        # 确保是最小值的倍数
                        if lot_size > 0:
                ...
                if (
                    ...
                    if lot_size > 0:
                        ...
                        if size_decimal < min_decimal:
                            lots = int(
                                (min_decimal / lot_decimal).quantize(
                    ...
                    # 如果没有获取到合约信息，使用配置值
                    trading_pairs = self.config_mgr.get_config(
                        "trading", "trading_pairs"
                    )
                    pair_config = next(
                        (p for p in trading_pairs if p.get("symbol") == symbol), {}
                    )
            ...
            if order.status == OrderStatus.FILLED:
                ...
                if filled_price and filled_price > 0:
                    ...
                    if decision.action == "long":
                        # 做多：止损在下方，止盈在上方
                        actual_stop_loss = filled_price * (
                            1 - stop_loss_price_change_pct
                        )
                        actual_take_profit = filled_price * (
                            1 + take_profit_price_change_pct
                        )
                    elif decision.action == "short":
                        # 做空：止损在上方，止盈在下方
                        actual_stop_loss = filled_price * (
                    ...
                    if actual_stop_loss and actual_take_profit:
                        ...
                        if (
                            stop_loss_price
                            and abs(stop_loss_price - actual_stop_loss) > 0.0001
                        ):
                            self.logger.warning(
            ...
            self.logger.info(
                f"决策执行完成: {order.order_id}, 状态={order.status.value}, 耗时={execution_time:.2f}秒"
            )
    ...
    async def _cancel_existing_sl_tp_orders(
    ...
    async def _check_before_open_position(
        self, symbol: str, action: str, position_side: str
    ) -> str:
        """
        开仓前检查：检查持仓和委托
        ...
        try:
            ...
            if current_position:
                ...
                current_pos_size = float(
                    current_position.get("pos", 0)
                    or current_position.get("posSz", 0)
                    or 0
                )
                ...
                if current_pos_side and current_pos_side != position_side:
                    ...
                    try:
                        ...
                        if close_result:
                            ...
                            self.logger.error(f"❌ {symbol}: 平仓失败，终止开仓")
                            return "error"
                    except Exception as e:
                        self.logger.error(f"❌ {symbol}: 平仓异常，终止开仓: {e}")
                        return "error"
    ...
    async def _close_opposite_position(
        self, symbol: str, position: Dict[str, Any], position_side: str
    ) -> bool:
        """
        平仓相反方向的持仓
    ...
    async def _set_stop_loss_take_profit(
        ...
        try:
            ...
            order_size = (
                main_order.filled_size
                if main_order.filled_size > 0
                else main_order.size
            )
                ...
                if position_side == "long" or side == "buy":
                    ...
                    if take_profit_price > current_price:
                        try:
                            ...
                            if (
                                take_profit_result
                                and take_profit_result.get("code") == "0"
                            ):
                                self.logger.info(
    ...
    async def _update_stop_loss_take_profit(
        self,
        symbol: str,
        order: Order,
        stop_loss_price: float,
    ...
    def _calculate_slippage(
        self, order: Order, expected_price: Optional[float]
    ) -> float:
        """
        计算滑点
    ...
    def get_execution_stats(self) -> Dict[str, Any]:
        ...
        if stats["total_orders"] > 0:
            ...
            stats["avg_slippage"] = (
                stats["total_slippage"] / stats["successful_orders"]
