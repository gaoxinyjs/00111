...
class PositionCalculator:
    ...
    def __init__(self):
        ...
        self.trading_config = self.config_mgr.get_config(
            "trading", "trading_optimization", {}
        )

        # 仓位模式（固定USDT/比例/混合）
        self.position_mode = self.position_config.get(
            "mode", "percentage"
        )  # 默认比例模式

        # 固定USDT配置
        self.fixed_usdt_per_trade = self.position_config.get(
        ...
        # 比例模式配置（重新设计）
        self.base_position_size = self.position_config.get(
            "base_position_size", 0.04
        )  # 基础仓位4%
        self.max_position_size = self.position_config.get(
            "max_position_size", 0.08
        )  # 单笔最大仓位8%
        self.min_position_size = self.position_config.get(
            "min_position_size", 0.01
        )  # 单笔最小仓位1%
        self.max_total_position = self.position_config.get(
            "max_total_position", 0.50
        )  # 总仓位上限50%

        # 仓位分级（根据信心度）
        self.position_high_confidence = self.position_config.get(
            "position_high_confidence", 0.06
        )  # 高信心度：5-8%
        self.position_medium_confidence = self.position_config.get(
            "position_medium_confidence", 0.04
        )  # 中等信心度：3-5%
        self.position_low_confidence = self.position_config.get(
            "position_low_confidence", 0.02
        )  # 低信心度：1-3%
    ...
    def calculate_position(
        ...
        try:
            ...
            if self.position_mode == "fixed_usdt":
                ...
                # 应用限制
                final_usdt = max(
                    self.min_usdt_per_trade, min(final_usdt, self.max_usdt_per_trade)
                )
                ...
                try:
                    ...
                    okx_client = OKXClient.get_instance()
                        ...
                        if isinstance(balance_info, dict):
                            ...
                            if (
                                ...
                                available_balance = float(
                                    usdt_detail.get("availBal", 0)
                                    or usdt_detail.get("availEq", 0)
                                    or 0
                                )
                            else:
                                # 如果没有details，尝试直接获取
                                available_balance = float(
                                    balance_info.get("availBal", 0)
                                    or balance_info.get("availEq", 0)
                                    or 0
                                )
    ...
    def _calculate_confidence_coeff(
    ...
    def _calculate_entry_timing_coeff(
        self, signal: Signal, market_data: Dict[str, Any]
    ) -> float:
        """
        计算入场时机系数（基于入场评分）
    ...
    def _calculate_position_adjust_coeff(
        self, signal: Signal, current_position: Optional[Dict[str, Any]]
    ) -> float:
        """
        计算当前持仓调整系数
